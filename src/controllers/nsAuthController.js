...
class NetSuiteAuthService {
  ...
  generatePKCE() {
    const verifier = this.base64URLEncode(
      crypto.randomBytes(config.oAuth.pkce.codeVerifierLength)
    )
    const method = String(config.oAuth.pkce.hashMethod).toUpperCase() || "S256"
    const challenge = this.base64URLEncode(
      crypto.createHash(method).update(verifier).digest()
    )
  ...
  generateState(length = config.oAuth.stateLength || 32) {
  ...
  getAuthorizationUrl(sessionId, config, expressSession) {
    ...
    console.log("Generated PKCE challenge for session:", pkce)
    ...
    console.log(
      "Generated auth URL with PKCE challenge stored in session",
      authUrl
    )
    return authUrl
  }
  ...
  async exchangeCodeForTokens(code, expressSession) {
    ...
    console.log(
      "Found PKCE challenge in session, proceeding with token exchange"
    )
    ...
    try {
      ...
      const params = {
        ...
        code: code,
        redirect_uri:
          config.redirectUri || "http://localhost:3001/api/auth/callback",
        client_id: config.clientId,
        code_verifier: verifier,
      }
